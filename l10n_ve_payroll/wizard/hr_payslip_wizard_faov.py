    ...
    def generar_txt(self):
        if self:
            employee_ids = self.env['hr.employee'].search(
                [('active', '=', True), ('company_id', '=', self.company_id.id), ('contract_id', '!=', False)])
            if not employee_ids:
            ...
            for employee in employee_ids:
                ...
                txt = txt + str(employee.identification_id).replace('.','').replace('-','').replace('V','').replace('v','') + ","
                ...
                txt = txt + str(contrato.date_start.strftime('%d%m%Y')) + "\n"
