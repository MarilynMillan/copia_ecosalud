...
class HRContact(models.Model):
        ...
        for record in self:
            if record.currency_id_dif.rate:
                if record.currency_id_dif.name == record.currency_id.name:
                    record[("tax_today")] = 1
                if record.currency_id_dif.name != record.currency_id.name:
                    record[("tax_today")] = record.currency_id_dif.inverse_company_rate
    ...
    def _get_contract_work_entries_values(self, date_start, date_stop):
        ...
        res = super(HRContact, self)._get_contract_work_entries_values(date_start, date_stop)
        # mapear todos los employee_id unicos en la lista we
        employee_id = list(set([we['employee_id'] for we in res]))
        values_extra = []
        for we in employee_id:
            #mapear todos los employee_id unicos en la lista we
            #buscar el contrato de we
            contract = self.env['hr.contract'].search([('employee_id', '=', we)])

            #buscar hr.leave por horas extras
            ...
            for leave in hr_leave.filtered(lambda l: l.holiday_status_id.work_entry_type_id.id in [self.env.ref('l10n_ve_payroll.work_entry_type_HED').id, self.env.ref('l10n_ve_payroll.work_entry_type_HEN').id]):
                #crear ciclo para los dias desde leave.date_from hasta leave.date_to
                dias = (leave.date_to - leave.date_from).days + 1
                dia = leave.date_from.replace(hour=0, minute=0, second=0)
                request_hour_from = float(leave.request_hour_from) + 5
                request_hour_to = float(leave.request_hour_to) + 5

                for i in range(dias):
                    #crear work entry para cada dia
                    if leave.holiday_status_id.work_entry_type_id.id == self.env.ref('l10n_ve_payroll.work_entry_type_HED').id:
                        values_extra.append({
                            'name': 'Hora Extra Diurna: ' + contract.name,
                            'date_start': dia + timedelta(hours=request_hour_from),
                            'date_stop': dia + timedelta(hours=request_hour_to),
                            'work_entry_type_id': self.env.ref('l10n_ve_payroll.work_entry_type_HED').id,
                    ...
                    if leave.holiday_status_id.work_entry_type_id.id == self.env.ref('l10n_ve_payroll.work_entry_type_HEN').id:
                        values_extra.append({
                            'name': 'Hora Extra Nocturna: ' + contract.name,
                            'date_start': dia + timedelta(hours=request_hour_from),
                            'date_stop': dia + timedelta(hours=request_hour_to),
                            'work_entry_type_id': self.env.ref('l10n_ve_payroll.work_entry_type_HEN').id,
                            'contract_id': contract.id,
                            'employee_id': we,
                            'duration': leave.number_of_hours,
                            'leave_id': leave.id,
                    ...
                    dia = dia.replace(hour=0, minute=0, second=0)
                    dia = dia + timedelta(days=1)

        if values_extra:
            res.extend(values_extra)
        return res