...
class HRPayslip(models.Model):
    ...
    def _get_partner_id(self, salary_rule_id):
        if salary_rule_id.origin_partner == 'empleado':
            origin_partner = self.employee_id.work_contact_id.id
    ...
    def _create_account_move(self, values):
        ...
        for rec in res:
            ...
            if not default_account:
                raise UserError(_("La diario no tiene una cuenta por defecto."))
                ...
                if not verificar:
                    ...
                    if salario_integral > 0:
                        ...
                        data = {
                            ...
                            'monto_total': monto_presta + monto_adici,
    ...
    def send_to_sign(self):
        for rec in self:
            ...
            if rec.employee_id.work_email:
                ...
                pdf = self.env['ir.actions.report'].with_context(lang=rec.employee_id.sudo().address_id.lang)._render_qweb_pdf(rec.struct_id.report_id, rec.id, data={'company_id': rec.company_id})[0]
                ...
                request = self.env['sign.request'].create({
                    ...
                    'cc_partner_ids': [(4, rec.employee_id.address_id.id)],
                })
