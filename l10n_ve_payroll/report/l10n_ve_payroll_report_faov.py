    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        ...
        if payslip_line_ids:
            ...
            for d in departamentos:
                ...
                meses_data = []
                total_integral = payslip_line_ids.filtered(lambda x: x.department_id.id == d.id and x.salary_rule_id.id in rule_employee_integral_ids).mapped(campo_total)
                total_retenido = payslip_line_ids.filtered(lambda x: x.department_id.id == d.id and x.salary_rule_id.id in rule_employee_retenido_ids).mapped(campo_total)
                total_aporte = payslip_line_ids.filtered(lambda x: x.department_id.id == d.id and x.salary_rule_id.id in rule_company_aporte_ids).mapped(campo_total)
                ...
                for l in lines_employee:
                    ...
                    meses_data = []
                    total_integral = payslip_line_ids.filtered(lambda x: x.employee_id.id == l.id and x.salary_rule_id.id in rule_employee_integral_ids).mapped(campo_total)
                    total_retenido = payslip_line_ids.filtered(lambda x: x.employee_id.id == l.id and x.salary_rule_id.id in rule_employee_retenido_ids).mapped(campo_total)
                    total_aporte = payslip_line_ids.filtered(lambda x: x.employee_id.id == l.id and x.salary_rule_id.id in rule_company_aporte_ids).mapped(campo_total)
                    total = sum(total_retenido) + sum(total_aporte)
            ...
            total_integral = payslip_line_ids.filtered(lambda x: x.salary_rule_id.id in rule_employee_integral_ids).mapped(campo_total)
            total_retenido = payslip_line_ids.filtered(lambda x: x.salary_rule_id.id in rule_employee_retenido_ids).mapped(campo_total)
            total_aporte = payslip_line_ids.filtered(lambda x: x.salary_rule_id.id in rule_company_aporte_ids).mapped(campo_total)
            total = sum(total_retenido) + sum(total_aporte)
