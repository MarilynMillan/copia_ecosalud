...
class VenezuelaReportCustomHandlerInces(models.AbstractModel):
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        ...
        if not currency_dif == self.env.company.currency_id.symbol:
            trm = 1 / self.env.company.currency_id_dif.inverse_company_rate
        lines = []
        ...
        if lines_inces:
            ...
            trm = round(self.env.company.currency_id_dif.inverse_company_rate, self.env.company.currency_id_dif.decimal_places)
            lines.append((x, {
