...
class VenezuelaReportCustomHandlerISLR(models.AbstractModel):
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        #         del previous_options['rule_employee_integral_ids']
        #     if 'rule_employee_retenido_ids' in previous_options:
        #         del previous_options['rule_employee_retenido_ids']
        #     if 'employee_ids' in previous_options:
        #         del previous_options['employee_ids']
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        ...
        if not currency_dif == self.env.company.currency_id.symbol:
            ...
            currency_company = self.env.company.currency_id_dif
        ...
        if rule_employee_integral_ids and rule_employee_retenido_ids and employee_ids:
            ...
            for l in lines_employee:
                ...
                if lines_payslip:
                    ...
                    periodo = ""
                    monto_obj_retencion = lines_payslip.filtered(lambda x: x.salary_rule_id.id in rule_employee_integral_ids).mapped(campo_total)
                    porcentaje = l.islr
                    impuesto_retenido = lines_payslip.filtered(lambda x: x.salary_rule_id.id in rule_employee_retenido_ids).mapped(campo_total)
                    ...
                    columns = [{'name': '', 'class': 'text'},
                               {'name': self.format_value(sum(monto_obj_retencion), currency=currency_company, blank_if_zero=True, figure_type='monetary'), 'class': 'numeric text-end'},
                               {'name': str(porcentaje) + ' %', 'class': 'numeric text-end'},
                               {'name': self.format_value(sum(impuesto_retenido), currency=currency_company, blank_if_zero=True, figure_type='monetary'), 'class': 'numeric text-end'},
                               {'name': self.format_value(monto_obj_retencion_acumulado, currency=currency_company, blank_if_zero=True, figure_type='monetary'), 'class': 'numeric text-end'},
                               {'name': self.format_value(impuesto_retenido_acumulado, currency=currency_company, blank_if_zero=True, figure_type='monetary'), 'class': 'numeric text-end'},
                               ]
                    ...
                    if line_id in options.get('unfolded_lines'):
                        ...
                        for m in meses:
                            ...
                            lines_payslip_mes_number = lines_payslip.filtered(lambda x: x.slip_id.date_from.strftime("%m") == m)
                            if lines_payslip_mes:
                                periodo = '%s - %s' % (m, lines_payslip_mes[0].slip_id.date_from.strftime("%Y"))
                                monto_obj_retencion_mes = lines_payslip_mes.filtered(lambda x: x.salary_rule_id.id in rule_employee_integral_ids).mapped(campo_total)
                                porcentaje_mes = l.islr
                                impuesto_retenido_mes = lines_payslip_mes.filtered(lambda x: x.salary_rule_id.id in rule_employee_retenido_ids).mapped(campo_total)
                                ...
                                columns = [{'name': periodo, 'class': 'text'},
                                           {'name': self.format_value(sum(monto_obj_retencion_mes), currency=currency_company, blank_if_zero=True, figure_type='monetary'), 'class': 'numeric text-end'},
                                           {'name': str(porcentaje_mes) + ' %', 'class': 'percentage'},
                                           {'name': self.format_value(sum(impuesto_retenido_mes), currency=currency_company, blank_if_zero=True, figure_type='monetary'), 'class': 'numeric text-end'},
                                           {'name': self.format_value(monto_obj_retencion_acumulado_mes, currency=currency_company, blank_if_zero=True, figure_type='monetary'), 'class': 'numeric text-end'},
                                           {'name': self.format_value(impuesto_retenido_acumulado_mes, currency=currency_company, blank_if_zero=True, figure_type='monetary'), 'class': 'numeric text-end'},
                                           ]

                                lines.append((x, {
                                    'id': '~periodo~%s%s' % (lines_payslip_mes[0].slip_id.date_from.strftime("%Y"),lines_payslip_mes[0].slip_id.date_from.strftime("%m")),
                                    'name': '',
                                    'level': 3,
                                    'unfoldable': False,
                                    #'unfolded': line_id in options.get('unfolded_lines'),
    ...
    def imprimir_planillas(self, options, export_to_pdf=True):
        company_id = self.env.context.get('company_id') or self.env.company.id
        company_id = self.env['res.company'].browse(company_id)
