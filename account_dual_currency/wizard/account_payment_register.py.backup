...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _get_wizard_values_from_batch(self, batch_result):
        ...
        if not self.tax_today:
            tax_today = lines[0].company_id.currency_id_dif.inverse_company_rate
        else:
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        analytic_distribution = self.line_ids.mapped('move_id').mapped('line_ids').filtered(lambda x: x.analytic_distribution)
        payment_vals = {
            ...
            'invoices': self.line_ids.mapped('move_id').ids,
            'analytic_distribution': analytic_distribution[0].analytic_distribution if analytic_distribution else False,
            'invoice_ids': self.line_ids.mapped('move_id').ids,
            'amount_total_usd': abs(sum(self.line_ids.mapped('move_id').mapped('amount_residual_usd'))),
            'amount_total_bs': abs(sum(self.line_ids.mapped('move_id').mapped('amount_residual_signed'))),
        }
    ...
    def _create_payments(self):
        ...
        payments = super(AccountPaymentRegister, self.with_context(mantener_bs=False))._create_payments()
        analytic_distribution = self.line_ids.mapped('move_id').mapped('line_ids').filtered(lambda x: x.analytic_distribution)
        to_write = {
            'mantener_bs': self.mantener_bs
        }
        if analytic_distribution:
            to_write['analytic_distribution'] = analytic_distribution[0].analytic_distribution
        payments.write(to_write)
        payments.move_id._verificar_pagos()
        payments.with_context(mantener_bs=False)._currency_equal()
        #payments._compute_amount_total()
        ...
        if self.source_amount == 0:
            ...
            ##print('self.currency_id.is_zero(self.payment_difference)', self.currency_id.is_zero(self.payment_difference))
