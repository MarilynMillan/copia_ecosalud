...
class StockLandedCost(models.Model):
    ...
    currency_id = fields.Many2one('res.currency', required=True, default=lambda self: self.env.user.company_id.currency_id, store=True)
    currency_id_dif = fields.Many2one(
        comodel_name="res.currency",
        required=True,
        default=lambda self: self.env.user.company_id.currency_id_dif)
    ...
    amount_total_usd = fields.Monetary(
        'Total $', compute='_compute_total_amount_usd',
        store=True, tracking=True, currency_field="currency_id_dif")
    ...
    def _onchange_state(self):
        for rec in self:
            if rec.state == 'done':
                for p in self.valuation_adjustment_lines:
                   if p.product_id:
                       p.product_id.with_company(self.company_id).write({'last_cost_usd': float(p.product_id.with_company(self.company_id).standard_price_usd)})

    @api.depends('cost_lines.price_unit_usd')
    def _compute_total_amount_usd(self):
        for cost in self:
            price_unit_usd = sum(line.price_unit_usd for line in cost.cost_lines)
            cost.amount_total_usd = price_unit_usd
        ...
        for cost in self:
            ...
            for line in cost.valuation_adjustment_lines.filtered(lambda line: line.move_id):
                ...
                if self.target_model == 'manufacturing':
                    ...
                    cost_to_add_usd = line.additional_landed_cost_usd
                else:
                    # Prorate the value at what's still in stock
                ...
                if line.move_id._is_in():
                    if line.move_id in self.mrp_production_ids.move_finished_ids:
                        product_qty_p = self.mrp_production_ids.filtered(lambda m: m.move_finished_ids == move).qty_produced
                        qty_out = product_qty_p
                    else:
                        qty_out = line.move_id.product_qty - remaining_qty
                elif line.move_id._is_out():
                    product_qty = line.move_id.product_qty
                    if line.move_id in self.mrp_production_ids.move_finished_ids:
                        product_qty = self.mrp_production_ids.filtered(lambda m: m.move_finished_ids == move).qty_produced
                    qty_out = product_qty
                ...
                move.write({'stock_valuation_layer_ids': [(6, None, valuation_layer.ids if valuation_layer else [])]})
    ...
    def get_valuation_lines(self):
        ...
        for move in self._get_targeted_move_ids():
            ...
            if move in self.mrp_production_ids.move_finished_ids:
                product_qty = self.mrp_production_ids.filtered(lambda m: m.move_finished_ids == move).qty_produced
            vals = {
                ...
                'volume': move.product_id.volume * product_qty
            }
            lines.append(vals)
    ...
    #     res = super().button_validate()
        #commit the landed cost
        #self.env.cr.commit()
        #buscar el ultimo precio de la fecha del producto y actualizarlo en el campo last_cost_usd
        #for p in self.valuation_adjustment_lines:
        #    if p.product_id:
        #        p.product_id.with_company(self.company_id).write({'last_cost_usd': float(p.product_id.with_company(self.company_id).standard_price_usd)})
    #    return res
