...
class AccountReport(models.AbstractModel):
    ...
    def _query_get(self, options, date_scope, domain=None):
        context_self = self.with_context(currency_dif=options.get('currency_dif'))
    ...
    def get_options(self, previous_options=None):
        ...
        self.with_context(currency_dif=currency_dif, currency_id_company_name=currency_id_company_name)
        new_context = {
            **self._context,
    ...
    def _prepare_lines_for_analytic_groupby(self):
        ...
        changed_equivalence_dict = {
            ...
            "balance_usd": sql.SQL("-amount_usd"),
            ...
            "debit_usd": sql.SQL("CASE WHEN (amount_usd < 0) THEN amount_usd else 0 END"),
            "credit_usd": sql.SQL("CASE WHEN (amount_usd > 0) THEN amount_usd else 0 END"),
        }
        selected_fields = []
        for fname in stored_fields:
            if fname in changed_equivalence_dict:
                ...
                selected_fields.append(
                    sql.SQL(f'to_jsonb(UNNEST(ARRAY[{analytic_cols}])) AS "account_move_line.analytic_distribution"'))
            else:
