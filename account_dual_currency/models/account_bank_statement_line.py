...
class AccountBankStatementLine(models.Model):
    ...
    tasa_referencia_statement = fields.Float(string="Tasa", store=True, digits='Dual_Currency_rate')
        ...
        for rec in self:
            ...
            if rec.date:
                ...
                if tasa_fecha:
                    ...
                    tasa_referencia_statement = self.env.company.currency_id_dif.inverse_company_rate
            else:
                tasa_referencia_statement = self.env.company.currency_id_dif.inverse_company_rate
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not 'tasa_referencia_statement' in vals or not 'tax_today' in vals:
                tasa = None
                ...
                if not tasa:
                    if 'journal_id' in vals:
                        journal = self.env['account.journal'].browse(vals['journal_id'])
                        tasa = journal.company_id.currency_id_dif.inverse_company_rate
    ...
    def _prepare_counterpart_move_line_vals(self, counterpart_vals, move_line=None):
        ...
        #print('VALORES contrapartida ', valores)
        return valores
