...
class PosSession(models.Model):
    ...
    def _get_pos_ui_res_currency_ref(self, params):
        """
        CORRECCIÓN ODOO 17:
        Retornamos la lista completa. El JS se encargará de procesarla.
        """
        # Usamos los parámetros definidos en _loader_params_res_currency_ref
        res_currency = self.env["res.currency"].search_read(**params["search_params"])
        return res_currency # Retorna la lista [ {...} ], NO el objeto { ... }

    def _loader_params_res_currency_ref(self):
        currency_id = self.config_id.show_currency.id or self.company_id.currency_id.id
        return {
            'search_params': {
                'domain': [('id', '=', currency_id)],
                'fields': ['name', 'symbol', 'position', 'rounding', 'decimal_places', 'rate'],
            },
        }
