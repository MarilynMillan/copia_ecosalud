...
class PosSession(models.Model):
    ...
    def _load_pos_data_models(self, config_id):
        ...
        if payment_method_model:
            payment_method_model['fields'].extend(['x_printer_code', 'x_igtf_percentage', 'x_is_foreign_exchange'])
            
        # Agregar campos a account.tax
        tax_model = next((m for m in models if m['model'] == 'account.tax'), None)
        if tax_model:
            tax_model['fields'].append('x_tipo_alicuota')
            
        # Agregar campos a res.partner
        partner_model = next((m for m in models if m['model'] == 'res.partner'), None)
        if partner_model:
            partner_model['fields'].append('company_type')
