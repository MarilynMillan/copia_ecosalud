...
patch(Payment.prototype, {
    setup(obj, options) {
        ...
        this.x_printer_code = this.x_printer_code || this.payment_method.x_printer_code;
    },
    init_from_JSON(json) {
...
patch(Order.prototype, {
    setup(obj, options) {
        super.setup(...arguments);
        this.num_factura = this.num_factura || false;
        this.impresa = this.impresa || false;
    },
    init_from_JSON(json) {
        super.init_from_JSON(...arguments);
...
patch(ClosePosPopup.prototype, {
    setup() {
        super.setup(...arguments);
        // Aplicamos el mixin manualmente o copiamos los métodos necesarios si el mixin no es compatible con patch directo
        // En Odoo 17, PrintingMixin devuelve una clase, así que instanciamos sus métodos en el prototype
        const mixinMethods = PrintingMixin(class { setup(){} }).prototype;
        Object.getOwnPropertyNames(mixinMethods).forEach(method => {
            if (method !== 'constructor' && method !== 'setup') {
                this[method] = mixinMethods[method].bind(this);
            }
        });
        // Ejecutamos el setup del mixin
        mixinMethods.setup.call(this);
    },
    openDetailsPopup() {
        this.state.zReport = ""
...
patch(ReceiptScreen.prototype, {
    ...
    get order() {
        // En v17 ReceiptScreen maneja la orden actual
        return this.currentOrder || this.env.pos.get_order();
    },
